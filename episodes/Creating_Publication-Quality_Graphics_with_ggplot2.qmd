---
title: "Software Carpentry: Creating Publication-Quality Graphics with ggplot2"
...
library(gapminder)
gapminder::gapminder[1:4,]
```

Plotting our data is one of the best ways to quickly explore it and the various relationships between variables.

There are three main plotting systems in R, the [base plotting system](https://www.statmethods.net/graphs/index.html), the [lattice](https://www.statmethods.net/advgraphs/trellis.html) package, and the {ggplot2} package.
...
```{r}
#| fig.alt: "Blank plot, before adding any mapping aesthetics to ggplot()"
...
```{r}
#| fig.alt: "Plotting area with axes for a scatter plot of life expectancy vs GDP, with no data points visible."
#| message: false
ggplot(data = gapminder, 
...
```{r}
#| fig.alt: "Scatter plot of life expectancy vs GDP per capita, now showing the data points."
#| message: false
ggplot(data = gapminder, 
       mapping = aes(x = gdpPercap, y = lifeExp)) +
  geom_point()
...
```{r}
#| fig.cap: "Binned scatterplot of life expectancy versus year showing how life expectancy has increased over time"
ggplot(data = gapminder, 
...
```{r}
#| fig.cap: "Binned scatterplot of life expectancy vs year with color-coded continents showing value of 'aes' function"
ggplot(data = gapminder, 
       mapping = aes(x = year, y = lifeExp, 
                     color=continent)) +
  geom_point()
...
```{r}
#| fig.alt: "Scatter plot of life expectancy vs GDP per capita with a trend line summarising the relationship between variables. The plot illustrates the possibilities for styling visualisations in ggplot2 with data points enlarged, coloured orange, and displayed without transparency."
ggplot(data = gapminder, 
       mapping = aes(x=year, y=lifeExp, 
                     group=country)) +
  geom_point() + 
...
```{r}
#| fig.cap: "Scatterplot of GDP vs life expectancy showing logarithmic x-axis data spread"
ggplot(data = gapminder, 
       mapping = aes(x = gdpPercap, y = lifeExp)) +
...
```{r}
#| fig.alt: "Scatter plot of life expectancy vs GDP per capita with a blue trend line summarising the relationship between variables, and gray shaded area indicating 95% confidence intervals for that trend line."
ggplot(data = gapminder, 
       mapping = aes(x = gdpPercap, y = lifeExp)) +
  geom_point(alpha = 0.5) + 
  scale_x_log10() + 
...
```{r}
#| fig.alt: "Scatter plot of life expectancy vs GDP per capita with a trend line summarising the relationship between variables. The blue trend line is slightly thicker than in the previous figure."
#| message: false
ggplot(data = gapminder, 
       mapping = aes(x = gdpPercap, y = lifeExp)) +
  geom_point(alpha = 0.5) + 
...
This is a taste of what you can do with ggplot2. RStudio provides a really useful [cheat sheet](https://www.rstudio.org/links/data_visualization_cheat_sheet) of the different layers available, and more extensive documentation is available on the [ggplot2 website](https://ggplot2.tidyverse.org/reference/). All RStudio cheat sheets are available from the [RStudio website](https://www.rstudio.com/resources/cheatsheets/).
...
Here a possible solution: `xlab()` and `ylab()` set labels for the x and y axes, respectively
